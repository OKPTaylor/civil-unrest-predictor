...
def get_csv(csv_name):
    ...
    print(f"CSV found")
    return df
# call should be: wg.get_csv("csv_name")
'''---------------------------------------------------------------------- PREP --------------------------------------------------------------------------------'''
#fix everything in one go example used in project app predictor
def prep_it(df_name):
...
#call should be: wg.encode(df_name, col_list)
...
#plots pairwise relationship of one feature with the target variable and independent T-test   

def single_pairwise_ind(df_train_name, target_var, col):
    print(f"Null Hypothesis: There is no linear correlation between {col.upper()} and {target_var}")
    print(f"{col.upper()} and {target_var}")
    sns.lmplot(data=df_train_name, x=col, y=target_var,
            line_kws={'color':'red'})
    plt.show()
    #runs a two sample t-test on feature and target variable
...
#relationship of one feature with the target variable and independent T-test text only        
def single_pairwise_ind_text(df_train_name, target_var, col, pass_stat_list):
...
def single_pairwise_ind_text_1(df_train_name, target_var, col, pass_stat_list=[]):
    ...
    if p < 0.05:
        pass_stat_list=[]
        pass_stat_list.append(col_name)
        pass_stat_list.append(p)
        pass_stat_list.append("Reject the null hypothesis")
        #turns the list into a data frame
        pass_stat_list = pd.DataFrame(np.array(pass_stat_list).reshape(1,3), columns = ['Feature', 'p-value', 'Hypothesis'])
    if p > 0.05:
        pass_stat_list=[]   
        pass_stat_list.append(col_name)
        pass_stat_list.append(p)
        pass_stat_list.append("Fail to reject the null hypothesis")
        #turns the list into a data frame
        pass_stat_list = pd.DataFrame(np.array(pass_stat_list).reshape(1,3), columns = ['Variable', 'P-Value', 'Result'])
...
#a function that tests for normality of target variable using the Shapiro-Wilk test and only the passed ones to a list
def normality_test_list(df_train_name, target_var, list_of_passed_features=[], list_of_failed_features=[]):
    alpha = 0.05
    stat, p = stats.shapiro(df_train_name[target_var])
   
    if p > alpha:
        
        list_of_passed_features.append(target_var)
    else:
        list_of_failed_features.append(target_var)
    return list_of_passed_features, list_of_failed_features
               

#a function that runs through a list for shapiro test
def normality_test_loop(df_train_name):
    col_cat, col_num = cat_and_num_lists(df_train_name)
    for col in col_num:
        print()
        print(f"Null Hypothesis: {col.upper()} is normally distributed")
        print()
        print(f"{col.upper()}")
        sns.distplot(df_train_name[col])
        plt.title(f"{col.lower().replace('_',' ')}")
        plt.show()
        print()
        print('ANALYZE and SUMMARIZE')
        normality_test(df_train_name, col)
        print()
        print("--------------------------------------------------------")
